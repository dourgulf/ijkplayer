...
static int get_intvalue_from_meta(AVDictionary *m, const char *key) {
    if (m) {
        AVDictionaryEntry *e = av_dict_get(m, key, NULL, AV_DICT_IGNORE_SUFFIX);
        if (e) {
            return atoi(e->value);
        }
    }
...
static int read_thread(void *arg)
{
    ...
    log_timestamp("avformat_find_stream_info begin");
    err = avformat_find_stream_info(ic, opts);
    log_timestamp("avformat_find_stream_info end");
